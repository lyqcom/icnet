...
from dataset.segbase import SegmentationDataset
...
class CityscapesDataset(SegmentationDataset):
    ...
    """
    input_transform = tc.Compose([#组成一个变换列表
        transforms.ToTensor(),
        transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))])

    def __init__(self, root='/data/cityscapes/', split='train', base_size=1024, crop_size=720, mode='train', transform=
    input_transform):
        """
        Parameters
            root : string
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            img, mask = self._sync_transform(img, mask)#随机裁剪->np.array（像素值）
        ...
        # if self.transform is not None:
        img = self.transform(img)#->Tensor->mormalize
        # mask = self._class_to_index(mask)
        # print("img3: ", img[0].shape)
        # print("img:", img)
        # print("mask", mask)
        # print("----------------------")
        # img = img.squeeze(1)
...
def _get_city_pairs(folder, split='train'):#返回数据集img和mask两个路径数组
...
if __name__ == '__main__':
    ...
    # for data in train_ds.create_dict_iterator(output_numpy=True):
    #     print(data["imgs"].shape, data["masks"])
    # #     break
    pass
