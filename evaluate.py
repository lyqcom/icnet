...
from utils import ICNetLoss, SegmentationMetric, SetupLogger  # IterationPolyLR,
...
class Evaluator(object):
    def __init__(self, cfg):
        ...
        # load ckpt
        ckpt_file_name = '/root/bigpingping/ckpt0/ICNet-190_106.ckpt'
        param_dict = load_checkpoint(ckpt_file_name)
        load_param_into_net(self.model, param_dict)
        # pretrained_net = (cfg["test"]["ckpt_path"])
    ...
    def eval(self):
        ...
        metric = nn.ConfusionMatrix(num_classes + 1, normalize="no_norm", threshold=0.5)
        ...
        for i in range(len(self.image_paths)):
            ...
            # # save pred
            # pred = ms.ops.Argmax(outputs[0], 1)
    ...
    def _mask_transform(self, mask):
        mask = self._class_to_index(np.array(mask).astype('int32'))
        return ms.Tensor(np.array(mask).astype('int32'))  # torch.LongTensor
